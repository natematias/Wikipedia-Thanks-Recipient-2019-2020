...
### Date variables (four of them, only one is the lag baseline)

- **`created.dt`**: randomization date (when the participant was added to the study system).
  - Caveat: First-wave control participants have `created.dt` 38 days later than their treated partner due to a DB ingestion artifact. **Do not use as a lag baseline.**
- **`behavior.start.dt`**: when the volunteer **actually sent** the first-generation thank to the treated member of the matched pair. **Identical for both members of every matched pair. This is the correct lag baseline** for any "after thanks" analysis.
- **`first.gen.thank.ts`** (in the second-gen file only): `created_at` of the CivilServant ExperimentThing for the first-gen event ≈ enrollment date. **Not** the thanks-receipt date. Do not use as a lag baseline.
- **`second.gen.thank.ts`**: when the second-generation thank was sent. The behavioral outcome.
...
- `bin_deleted` in `prev.experience.treatment`: account was deleted before treatment could be delivered.
...
- `prev.experience.assignment.post.candidate` (str): same level, recomputed at the moment the participant was about to be shown to a volunteer.
...
- `created.dt` (datetime): randomization date. (See first-wave 38-day caveat above.)
- `removed.dt` (datetime | NA): when the participant was removed from the study (GDPR request, account deletion).
- `behavior.start.dt` (datetime): when the volunteer actually sent first-gen thanks. **Use this for lag analyses.**
...
- `user.registration.dt.sync.object` (datetime): same as above; retained for parity with upstream audit fields.
- `created.dt.sync.object` (datetime): when the matched-pair `sync_object` was created in the CivilServant DB.

### Pre- / post-treatment outcomes

The 84-day pre-window ends at `behavior.start.dt`. The post-window is the 84 days starting at `behavior.start.dt` for retention and labor hours; cumulative thanks-sent counts span the full post-treatment study period.

- `two.week.retention` (bool): edited at least once during days 1–14 after `behavior.start.dt`.
- `labor.hours.pre.treatment` (num): total estimated labor hours over the 84-day pre-window. Computed by summing inter-edit durations capped at 1 hour per gap.
- `labor.hours.post.treatment` (num): same, post-window.
...
### Treatment-delivery audit

- `number.thanks.received` (int): number of first-gen thanks received during the study. By design, 0 for control; ≥ 1 for the 31% of treated newcomers / 43% of treated experienced who were actually selected and thanked.
- `number.skips.received` (int): times a volunteer was shown this participant in the app but clicked *Skip*.
- `num.messages` (int): times a volunteer-thanker sent a message via the app's UI (separate from the MediaWiki Thanks extension).
